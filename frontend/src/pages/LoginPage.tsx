      ...
      <div className="relative z-10 w-full max-w-sm mx-4">
        ...
        >
          ...
          {error && (
            ...
            >
              {error}
              <button onClick={clearError} className="float-right opacity-60 hover:opacity-100">✕</button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            >
              {isLoading ? 'Connecting...' : 'Enter the Protocol'}
            </button>
          </form>
          ...
          <div className="mt-6 text-center text-xs" style={{ color: '#4a5568', fontFamily: 'JetBrains Mono, monospace' }}>
            No account?{' '}
            <Link to="/register" className="transition-colors" style={{ color: '#00d4ff' }}>
              Register →
            </Link>
          </div>
